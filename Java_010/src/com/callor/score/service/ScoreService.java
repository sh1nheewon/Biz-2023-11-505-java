...
public class ScoreService {
	...
	/*
	 * inoutScore() method 를 매개변수 없이 호출하면 method 내부에서 
	 * start 값과 end 값 임의로 생성하고 
	 * 현재 class 에 있는 
	 * 		inputScroe(int start,int end) method 에게 
	 * 		전달하기
	 */
	public void inputScore() { //점수 범위 상관 X
		int start = 0;
		int end = 100;
	...
	public void inputScore(int start,int end) { // 점수 범위 설정
		...
		// scores 리스트에 저장되어 있는 데이터 중 가장 마지막 요소(제일끝 index 요소)에서 
		// 학번을 가져와서 strStdNum 변수에 할당하라 (scores.size() - 1 ) : 배열번호
//		String strStdNum = "";
//		if(scores.size() > 0) {
...
//		}
		...
		// 만약 scores 리스트에 데이터가 한개라도 있으면 마지막 요소의 학번을 strStdNum 에 할당하고
		// 그렇지 않으면 strStdNum 의 초기값인 230000 을 그대로 유지하라
