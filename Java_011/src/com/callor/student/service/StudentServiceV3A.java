...
public class StudentServiceV3A {
	...
	/*
	 * 학번을 매개변수로 전달받아 
	 * students List 에서 검색하여 
	 * 일치하는 학생정보가 있으면 그 정보를 통채로(dto) return
	 */
	private StudentDto selectStdNum(String num) {
		/*
		 * students 는 List type students 의 요소는 StudentDto type 이 명령은 students List 개수만큼
		 * for 반복하기. 반복하면서 students List 의 개별 요소를 get(읽기) 하여 StudentDto type 인 dto 에
		 * 할당하여 코드블럭( {} )에 전달하라. 
		 * // students list 개수만큼 반복. 그 정보를 dto 에 저장
		 */
