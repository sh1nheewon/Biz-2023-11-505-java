...
public class ScoreServiceV1 {
	 ...
	 * 이때 V1 에 선언된 public method 는 별다른 코드변경 없이 V2 에서 그대로 사용할 수 있다
	 * 하지만 private 으로 선언된 변수는 상속이 안된다
	 * scores 객체를 V2 에서 사용하기 위하여 protected 로 변경해 준다
	 */
	protected List<ScoreDto> scores = null;

	public ScoreServiceV1() {
		// 생성자에서 아래와 같이 scores 를 생성하면 생성자를 벗어난 후 scores 는 소멸된다
	...
	public void makeScore() {
		...
		/*
		 * 학번을 자동으로 생성하기 1. scores 에 추가된 성적표가 있는가 확인 2. 없으면 최초의 성적이므로 230001 로 학번 세팅 3.
		 * 있으면 가장 마지막 성적표의 학번을 조회하여 학번을 +1 하여 새로운 학번을 생성하기
		 */
		String strStdNum = "23001";
		int scoresSize = scores.size();
		if (scoresSize > 0) { // strStdNum 값이 23000 이면 if문 실행 x
	...
	// count 변수에 정수값을 전달받아 count 만큼 makeScore() 호출하여
